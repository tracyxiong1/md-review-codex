...
4. 本地执行可用时，自动启动 `md-review-server`。迭代评审优先服务父目录，使新生成的 `v2`、`v3` 可在文件树中直接切换：
...
6. 将 `3030` 视为首选起始端口，不视为固定端口。`md-review-server` 会在请求端口被占用时自动尝试后续端口，服务输出会打印实际 URL。后续链接和 API 调用必须使用实际端口。
7. 只有在确认已有 `md-review-server` 服务正在服务正确文件或父目录时，才复用已有服务。
